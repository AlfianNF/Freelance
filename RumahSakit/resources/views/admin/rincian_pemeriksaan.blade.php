...
@foreach ($rincian_jenis_pemeriksaan as $rincian)
    <div id="modal_{{ $rincian->id }}"  class="modal">
      <div class="modal-content">
        <span id="closeModalBtn" class="close">&times;</span>
        <div class="isi_modals">
          <h1 class="text">Edit Jenis Pemeriksaan </h1>
          <form action="{{ route('dashboard.admin.update_rincian_pemeriksaan', ['id' => $rincian->id]) }}" method="POST">
            @csrf @method('PUT')

            <label class="text1" for="nama_jenis_pemeriksaan">Rincian Jenis Pemeriksaan :</label>
            <input class="input" type="text" id="nama_jenis_pemeriksaan" name="nama_jenis_pemeriksaan" value="{{ $rincian->nama_jenis_pemeriksaan }}" />

            <input type="hidden" name="id_jenis_pemeriksaan" id="id_jenis_pemeriksaan" value="{{ $rincian->id_jenis_pemeriksaan }}">
        </div>
        <button class="btn_close" >Update</button>
      </div>
    </form>
    </div>
    @endforeach
    ...
    <div class="Dasar">
        <div class="kiri">
          ...
          <div class="bawah-kiri">
            ...
            ><br />
            <a href="/dashboard/admin/rincian_pemeriksaan" class="aktif_btn">
      ...
      <div class="kanan">
        <div class="atas-kanan">
          ...
          <form action="/dashboard/admin/rincian_pemeriksaan" method="GET">
        ...
        <div class="bawah-kanan">
          <div class="sampul">
            <div class="judul">
              <span class="desjudul">Tambah Data Rincian Jenis Pemeriksaan</span>
                <form action="{{ route('dashboard.admin.rincian_pemeriksaan') }}" method="POST">
                @csrf
                <label class="text" for="jenis_pemeriksaan">Jenis Pemeriksaan</label>
                <select class="select" name="id_jenis_pemeriksaan" id="id_jenis_pemeriksaan">
                    @foreach ($jenis_pemeriksaan as $pemeriksaan)         
                        <option value="{{ $pemeriksaan->id }}">{{ $pemeriksaan->jenis_pemeriksaan }}</option>
                    @endforeach
                </select>

                <label class="text" for="nama_jenis_pemeriksaan">Rincian Jenis Pemeriksaan</label>
                <input class="input" type="text" name="nama_jenis_pemeriksaan" id="nama_jenis_pemeriksaan" placeholder="Masukan Rincian Jenis Pemeriksaan">

                <button class="btn_submit" type="submit" name="kirim"><span class="text">Submit</span></button>
                </form>
            </div>
            <div class="konten">
              ...
              <table class="table-bordered">
                <thead>
                    <tr class="thead">
                        ...
                        <th>Rincian Jenis Pemeriksaan</th>
                ...
                <tbody>
                    ...
                    @foreach ($rincian_jenis_pemeriksaan as $rincian) 
                    <tr>
                        <td><center>{{ $loop->iteration + $rincian_jenis_pemeriksaan->firstItem() - 1  }}</center></td>
                        <td><center>{{ $rincian->jenisPemeriksaan->jenis_pemeriksaan }}</center></td>
                        <td><center>{{ $rincian->nama_jenis_pemeriksaan }}</center></td>
                        <td class="button-container">
                            <div class="button-group">
                                <button id="openModalBtn_{{ $rincian->id }}" class="btn_edit"><span class="text">Edit</span></button>
                                <form action="{{ route('dashboard.admin.delete_rincian_pemeriksaan', ['id' => $rincian->id]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="paginate">
              <ul class="pagination">
                  <!-- Tombol "Previous" -->
                  @if ($rincian_jenis_pemeriksaan ->currentPage() > 1)
                      <li><a href="{{ $rincian_jenis_pemeriksaan ->previousPageUrl() }}">Previous</a></li>
                  @endif
          
                  <!-- Nomor halaman 1-5 -->
                  @php
                      // Hitung nomor halaman yang akan ditampilkan
                      $startPage = $rincian_jenis_pemeriksaan ->currentPage() >= 5 ? $rincian_jenis_pemeriksaan ->currentPage() - 3 : 1;
                      $endPage = min($rincian_jenis_pemeriksaan ->lastPage(), $startPage + 4);
                  @endphp
                  @for ($i = $startPage; $i <= $endPage; $i++)
                      <li class="{{ $rincian_jenis_pemeriksaan ->currentPage() == $i ? 'active' : '' }}">
                          <a href="{{ $rincian_jenis_pemeriksaan ->url($i) }}">{{ $i }}</a>
                      </li>
                  @endfor
          
                  <!-- Tombol "Next" jika halaman berikutnya ada -->
                  @if ($rincian_jenis_pemeriksaan ->currentPage() < $rincian_jenis_pemeriksaan ->lastPage())
                      <li><a href="{{ $rincian_jenis_pemeriksaan ->nextPageUrl() }}">Next</a></li>
                  @endif
              </ul>
          </div>
          </div>
